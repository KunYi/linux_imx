...
static int I2C_RxData(uint16_t slaveAddr, uint8_t cmd, uint8_t *rxData, int length)
{
	...
	{
		printk(KERN_ERR "[PS_ERR][CM36771 error] %s retry over %d\n",
...
static int cm36771_i2c_read_word(uint16_t slaveAddr, uint8_t cmd, uint16_t *pdata)
{
	...
	{
		pr_err(
			"[PS_ERR][CM36771 error]%s: I2C_RxData fail [0x%x, 0x%x]\n",
...
#if 0
	/* Debug use */
	printk(KERN_DEBUG "[CM36771] %s: I2C_RxData[0x%x, 0x%x] = 0x%x\n",
...
static int cm36771_i2c_write_word(uint16_t SlaveAddress, uint8_t cmd, uint16_t data)
{
	...
	/* Debug use */
	printk(KERN_DEBUG
	"[CM36771] %s: cm36771_i2c_write_word[0x%x, 0x%x, 0x%x]\n",
...
static int get_ps_adc_value(uint16_t *data)
{
	...
	{
		pr_err(
			"[PS][CM36771 error]%s: cm36771_i2c_read_word fail\n",
			__func__);
		return -EIO;
	}
	else
	{
		pr_err(
			"[PS][CM36771 OK]%s: cm36771_i2c_read_word OK 0x%x\n",
			__func__, *data);
	}
...
static int lightsensor_get_cal_data(struct cm36771_info *lpi)
{
	...
	pr_info("[LS][CM36771] %s: cal_data = %d\n",
				...
				const char *buf, size_t count)
{
	...
	printk(KERN_INFO
		"[PS][CM36771]Set PS_close_thd_set = 0x%x, PS_away_thd_set = 0x%x, PS_cmd_cmd:value = 0x%x\n",
		lpi->ps_close_thd_set, lpi->ps_away_thd_set, PS_cmd_test_value);
...
static int lightsensor_setup(struct cm36771_info *lpi)
{
	...
	{
		pr_err(
			"[LS][CM36771 error]%s: could not allocate ls input device\n",
			__func__);
		return -ENOMEM;
	}
...
static int psensor_setup(struct cm36771_info *lpi)
{
	...
	{
		pr_err(
			"[PS][CM36771 error]%s: could not allocate ps input device\n",
			__func__);
		return -ENOMEM;
	}
	...
	{
		pr_err(
			"[PS][CM36771 error]%s: could not register ps input device\n",
			__func__);
	...
	{
		pr_err(
			"[PS][CM36771 error]%s: could not register ps misc device\n",
			__func__);
...
static int cm36771_setup(struct cm36771_info *lpi)
{
	...
	{
		pr_err(
			"[PS][CM36771 error]%s: req_irq(%d) fail for gpio %d (%d)\n",
			__func__, lpi->irq,
			lpi->intr_pin, ret);
		goto fail_free_intr_pin;
	}
